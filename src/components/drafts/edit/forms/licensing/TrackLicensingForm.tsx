...
interface TrackLicensingFormProps {
  draft: Draft;
  onChange: (updates: Partial<Draft>) => void;
}

export function TrackLicensingForm({ draft, onChange }: TrackLicensingFormProps) {
  const [selectedTier, setSelectedTier] = useState<LicenseTier>(draft.licensing?.tier || 'standard');
  
  // Initialize licensing data if not present
      ...
      <div className="flex-1 space-y-6 overflow-y-auto px-6 pb-6">
        ...
        {licensing.requiresLicense && (
          <>
            ...
            {/* Custom Price (only for bespoke tier) */}
            {selectedTier === 'bespoke' && (
              <div className="space-y-2">
            ...
            {/* Restrictions */}
            <div className="space-y-2">
              <Label>Licensing Restrictions</Label>
              <Textarea
                placeholder="Enter any specific restrictions or limitations..."
                value={licensing.restrictions}
                onChange={(e) => 
                  handleLicensingChange({ restrictions: e.target.value })
                }
              />
            </div>
          </>
